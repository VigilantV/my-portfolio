...
const Projects = ({ currentSectionIndex, navigateToSection }) => {
  ...
  const [enableFramer, setEnableFramer] = useState(false);
  ...
  useEffect(() => {
    ...
    if (currentSectionIndex === 1 && !hasVisitedRef.current) {
      ...
      setEnableFramer(true);
    }
  }, [currentSectionIndex, titleAnimated, enableFramer]);

  const sectionInitial = enableFramer ? "hidden" : false;
  const sectionAnimate = enableFramer
    ? currentSectionIndex === 1
      ? "visible"
      : "hidden"
    : undefined;
